package com.dncd.apt.processor

import com.dncd.apt.anns.TestApt
import com.dncd.apt.utils.PrintAptUtil
...
@SupportedAnnotationTypes(
    "com.dncd.apt.anns.TestApt"
) //标注注解处理器支持的注解类型，就是我们刚才定义的接口Test，可以写入多个注解类型。
class TestAptProcessor : AbstractProcessor() {
    ...
    override fun process(p0: MutableSet<out TypeElement>, roundEnvironment: RoundEnvironment): Boolean {
        ...
        val methodMain = MethodSpec.methodBuilder("main") //创建main方法
        ...
        val helloWorld = TypeSpec.classBuilder("HelloWorld") //创建HelloWorld类
            ...
            .build()
        val javaFile = JavaFile.builder("com.dncd.apt", helloWorld).build() // 生成源   代码
